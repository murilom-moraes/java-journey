...
* Enumerações e Composição: Modelos de domínio e relacionamento entre objetos.
* Herança e Polimorfismo: Reuso de código, sobreposição e classes/métodos abstratos.
* Tratamento de Exceções: Estruturas `try-catch`, blocos `finally` e exceções customizadas.
* Manipulação de Arquivos: Leitura e escrita de dados em arquivos de texto.
* Interfaces e Generics: Desacoplamento de código e coleções genéricas (`Set`, `Map`).
* Programação Funcional: Expressões Lambda, interfaces funcionais e Stream API.
* JDBC & Persistência: Conexão com bancos de dados relacionais.
* JPA / Hibernate & Spring Boot: Mapeamento Objeto-Relacional e desenvolvimento de APIs RESTful.
